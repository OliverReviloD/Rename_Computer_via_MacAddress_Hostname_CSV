...
$cCSVFileReadAccount	  =  $cComputerRenameAccount                    #   "MyDomain\ReaderAccount"
$cCSVFileReadPWD		  =  $cComputerRenamePWD                        #   "P@ssw0rd"
...
Function Main
    ...
    if ( $PsDriveAlreadyConnected -eq $Null -and $SmbMappingAlreadyConnected -eq $Null )
        ...
        if ( $NewPsDrive -eq $Null) 
            ...
            Logging "$cAppName - ERROR - Script will terminate with error 0x80004005 - 'The specified network share does not exist'   or  'The specified network account or password are not correct' "
            Return -2147467259  # = 0x80004005   -  "The specified network share does not exist"   or  "The specified network password is not correct"
            }
    ...
    # ( $AllNetIPConfigurations | Measure-Object ).count
    if (   ( $AllNetIPConfigurations | Measure-Object ).count   -gt 1 )
        {
        $AllNetIPConfigurations | FT | out-string | Logging 
        }
    else
        { 
        Logging "$($AllNetIPConfigurations.MacAddress)   $($AllNetIPConfigurations.IPv4Address)    $($AllNetIPConfigurations.InterfaceAlias)   $($AllNetIPConfigurations.InterfaceDescription)" 
...
#>
    ...
    Logging "Join_Domain_Or_Workgroup() - END - because not transcoded from VBS to PS1"
    return
...
Function Disconnect_PsDrive__if_required 
    {
    Param ($StatusBefore , $StatusNow, [String]$DisconnectDriveLetter , [Switch]$MyDebug)
    ...
    if ( $StatusBefore -eq $Null -and $StatusNow -ne $Null )
        ...
        {
        Logging "$FctName no disconnect required"
        }
    }   
...
#>
    ...
    $DetectedRow = Import-Csv -Delimiter $Delimter   -Path $CsvFilePath -Header $Header  | Where-Object -Property MAC -eq $MACADDRESS
    If ( $DetectedRow ) { $NewComputerName= $DetectedRow.ComputerName }
    ...
    If ( $NewComputerName -eq $Null) { Logging "$FctName new Computername is NOT found in CSV file!"  } else {  Logging "$FctName new computername found '$NewComputerName'"  }
